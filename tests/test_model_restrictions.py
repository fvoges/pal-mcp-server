...
class TestProviderIntegration:
    ...
    @patch.dict(os.environ, {"GOOGLE_ALLOWED_MODELS": "gemini-3.8-flash"})
...
class TestShorthandRestrictions:
    ...
    def test_providers_validate_shorthands_correctly(self):
        ...
        with patch.object(ModelProviderRegistry, "get_provider", side_effect=registry_side_effect):
            ...
            assert not openai_provider.validate_model_name("gpt-5-mini")  # Legacy mini is unrelated
            ...
            assert not gemini_provider.validate_model_name("gemini-2.5-flash")  # Legacy flash is unrelated
...
class TestAutoModeWithRestrictions:
    ...
    def test_fallback_with_shorthand_restrictions(self, monkeypatch):
        ...
        try:
            ...
            # "mini" is now an alias for gpt-5.4-mini, but the list may show "mini" itself
            assert model in ["mini", "gpt-5.4-mini", "o4-mini", "gemini-3.8-flash"]
        finally:
